...
inline uint32_t internal_normalize(uint32_t sign, int32_t exp, uint64_t sig_grs)
{
	...
	if ((sig_grs >> (23 + 3)) > 1 || exp < 0)
	{
		...
		if (exp == 0)
		{
			...
			/* TODO: shift right, pay attention to sticky bit*/
			sticky = sticky | (sig_grs & 0x1);//keep sticky while
			sig_grs = sig_grs >> 1;
			sig_grs |= sticky;
		}
		if (exp < 0)
	...
	if (!overflow)
	{
		...
		sig_grs=sig_grs>>3;
	    if(grs>0x4 || (sig_grs%2!=0&&grs==0x4))
	        sig_grs++;
        if(sig_grs >> 23 >1){
            sig_grs=sig_grs>>1;
            exp++;
...
uint32_t internal_float_add(uint32_t b, uint32_t a)
{
	...
	if (fa.exponent > fb.exponent)//ensure a.exp<b.exp
	{
		fa.val = b;
		fb.val = a;
	}
	...
	sig_a = fa.fraction;
	int flaga=0,flagb=0;//flaga flagb the same shift right
	if (fa.exponent != 0)
		sig_a |= 0x800000; // the implied bit
	else
	    flaga=1;
	sig_b = fb.fraction;
	if (fb.exponent != 0)
	...
	uint32_t shift = fb.exponent-fa.exponent - (flaga^flagb);
	
	/* TODO: shift = b.exp-a.exp */
	...
	while (shift > 0)
	{
		sticky = sticky | (sig_a & 0x1);//keep sticky while
		sig_a = sig_a >> 1;
		sig_a |= sticky;
		shift--;
	}
