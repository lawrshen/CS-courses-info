...
cmd_handler(cmd_x){
    ...
    if (!success)
	{
	    ...
	    for(int i=0;i<print_num;i+=4,addr+=16){
	        printf("0x%08x: ",addr);
	        printf(" %08x ",vaddr_read(addr,SREG_CS,4));
	        printf(" %08x ",vaddr_read(addr+4,SREG_CS,4));
	        printf(" %08x ",vaddr_read(addr+8,SREG_CS,4));
	        printf(" %08x\n",vaddr_read(addr+12,SREG_CS,4));
	    }
	    fflush(stdout);
	}
	return 0;
